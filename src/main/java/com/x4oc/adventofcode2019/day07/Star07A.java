...
import static com.x4oc.adventofcode2019.day07.Star07A.Comparison.*;
import static com.x4oc.adventofcode2019.day07.Star07A.Operation.*;

public class Star07A {
    private static final Logger logger = LoggerFactory.getLogger(Star07A.class);
    ...
    public static void execute(){
        ...
        try (Stream<String> stream = Files.lines(Paths.get(fileName))) {
            ...
            OptionalInt maxThrust = Generator.permutation(5, 6, 7, 8, 9)
                    .simple()
                    .stream()
                    .mapToInt(phaseSetting -> calculateThrust(opcodes, phaseSetting))
                    .max();
    ...
    private static AmplifierState calculate(List<Integer> opcodes, int input, int phase, int position, boolean firstInput) {
        boolean halt = false;
        boolean haltLast = false;
        int thrust = 0;
        do{
            ...
            switch (opcode % 10){
                ...
                case 3:
                    ...
                    break;
                case 4:
                    thrust = getOperand(opcodes, position + 1, 0);
                    halt = true;
                    position += 2;
                    break;
                case 5:
        ...
        return new AmplifierState(opcodes, position, thrust, haltLast);
    }
    ...
    public static int calculateThrust(List<Integer> opcodes, List<Integer> phaseSetting){
        AmplifierState state = AmplifierState.emptyAmplifierState;

        Map<Integer, List<Integer>> opcodesByPhase = new HashMap<>();
        Map<Integer, Integer> positionByPhase = new HashMap<>();
        for(int phase : phaseSetting){
            opcodesByPhase.put(phase, new ArrayList<>(opcodes));
            positionByPhase.put(phase, 0);
        }
        ...
        do{
            for(int phase : phaseSetting){
                ...
                if(numberOfCalculations > 5){
                    firstInput = true;
                }
                state = calculate(opcodesByPhase.get(phase), state.getThrust(), phase, positionByPhase.get(phase), firstInput);
                opcodesByPhase.put(phase, state.getOpcodes());
                positionByPhase.put(phase, state.getPosition());
            }

        }while(!state.isHaltLast());

        return state.getThrust();
    }
